...
public class MySimpleActivity extends Activity implements
            OnClickListener {
            ...
            try {
                ...
                wv.setWebChromeClient(new WebChromeClient() {
                    public void onProgressChanged(WebView view, int progress) {
                        ...
                        MySimpleActivity.this.setTitle("Loading...");
                        MySimpleActivity.this.setProgress(progress * 100);

                        //set the title of the layout
                        if(progress == 100)
                            MySimpleActivity.this.setTitle(R.string.app_name);
                    }
                });
                ...
                switch (v.getId()) {
                    case R.id.bAddr:
                        ...
                        if (url != "") {
                            if(url != "https://www.wikipedia.org/" ){
                                url = URLUtil.isValidUrl(url) ? addr.getText().toString()   : "http://" + "www.he.m.wikipedia.org";
                                wv.loadUrl(url);
                                addr.setText(url);
                                wv.requestFocus();
                            }
                    ...
                    case R.id.bKeyword:
                        ...
                        if (key != "") {
                            if(key != "ויקיפדיה")
                                key = "http://www.google.com/search?q="+ "ויקפדיה";
                            wv.loadUrl(key);
                            addr.setText(key);
                            wv.requestFocus();
                        }
